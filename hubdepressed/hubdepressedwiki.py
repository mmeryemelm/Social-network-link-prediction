...
def hub_depressed(graph, node1, node2):
    ...
    if max_degree == 0:
        return 0
    hub_depression = len(common_neighbors) / max_degree
    return hub_depression

def get_predicted_edges(graph, node):
    predicted_edges = []
    for neighbor in graph.nodes():
        if neighbor != node and not graph.has_edge(node, neighbor):
            hub_depression = hub_depressed(graph, node, neighbor)
            if hub_depression > 0.2:  # Modify the threshold as desired
                predicted_edges.append((node, neighbor, hub_depression))
...
while True:
    ...
    if len(predicted_edges) == 0:
        ...
        for edge in predicted_edges:
            node1, node2, hub_depression = edge
            table_data.append([node1, node2, hub_depression])

        table = tabulate(table_data, headers=['Node 1', 'Node 2', 'Hub depression'], tablefmt='psql')
        print(table)
