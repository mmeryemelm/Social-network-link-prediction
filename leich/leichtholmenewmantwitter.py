...
def get_predicted_edges(graph, node):
    ...
    for neighbor in graph.nodes():
        if neighbor != node and not graph.has_edge(node, neighbor):
            index = leicht_holme_newman_index(graph, node, neighbor)
...
while True:
    ...
    if len(predicted_edges) == 0:
        ...
        # Generate the table with the predicted edges and Leicht-Holme-Newman Index
        table_data = []
        for edge in predicted_edges:
            node1, node2, index = edge
            table_data.append([node1, node2, index])

        table = tabulate(table_data, headers=['Node 1', 'Node 2', 'Leicht-Holme-Newman Index'], tablefmt='psql')
        print(table)
