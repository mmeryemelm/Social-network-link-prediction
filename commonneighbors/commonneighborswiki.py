...
def common_neighbors_ratio(graph, node1, node2):
    ...
    common_neighbors_ratio1 = len(common_neighbors)

    return common_neighbors_ratio1
...
def get_predicted_edges(graph, node):
    ...
    for neighbor in graph.nodes():
        if neighbor != node and not graph.has_edge(node, neighbor):
            ...
            if ratio > 0.2:  # Modify the threshold as desired
...
while True:
    ...
    if len(predicted_edges) == 0:
        ...
        # Generate the table with the predicted edges and ratio
        table_data = []
        for edge in predicted_edges:
            node1, node2, ratio = edge
            table_data.append([node1, node2, ratio])

        table = tabulate(table_data, headers=['Node 1', 'Node 2', 'Common Neighbors Ratio'], tablefmt='psql')
        print(table)
