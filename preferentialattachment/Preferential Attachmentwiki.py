...
def get_predicted_edges(graph, node):
    ...
    for neighbor in graph.nodes():
        if neighbor != node and not graph.has_edge(node, neighbor):
            attachment = preferential_attachment(graph, node, neighbor)
            if attachment > 10:  # Modify the threshold as desired
                predicted_edges.append((node, neighbor, attachment))
...
while True:
    ...
    if len(predicted_edges) == 0:
        ...
        # Generate the table with the predicted edges and attachment
        table_data = []
        for edge in predicted_edges:
            node1, node2, attachment = edge
            table_data.append([node1, node2, attachment])

        table = tabulate(table_data, headers=['Node 1', 'Node 2', 'Preferential Attachment'], tablefmt='psql')
        print(table)
